...
void sort(const emlrtStack *sp, emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  loop_ub = x->size[1];
  idx->size[1] = loop_ub;
  emxEnsureCapacity_int32_T(&st, idx, i1, &gb_emlrtRTEI);
  idx_data = idx->data;
  for (k = 0; k < loop_ub; k++) {
    idx_data[k] = 0;
  }
  b_st.site = &ad_emlrtRSI;
  emxInit_int32_T(&b_st, &iwork, 1, &hb_emlrtRTEI);
  i1 = iwork->size[0];
  iwork->size[0] = loop_ub;
  emxEnsureCapacity_int32_T(&b_st, iwork, i1, &hb_emlrtRTEI);
  iwork_data = iwork->data;
  for (k = 0; k < loop_ub; k++) {
    iwork_data[k] = 0;
  }
  emxInit_real_T(&b_st, &xwork, 1, &jb_emlrtRTEI);
  i1 = xwork->size[0];
  xwork->size[0] = loop_ub;
  emxEnsureCapacity_real_T(&b_st, xwork, i1, &ib_emlrtRTEI);
