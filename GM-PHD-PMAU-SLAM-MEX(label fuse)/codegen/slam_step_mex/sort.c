...
void sort(const emlrtStack *sp, emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  loop_ub = x->size[1];
  idx->size[1] = loop_ub;
  emxEnsureCapacity_int32_T(&st, idx, i1, &mb_emlrtRTEI);
  idx_data = idx->data;
  for (k = 0; k < loop_ub; k++) {
    idx_data[k] = 0;
  }
  b_st.site = &gd_emlrtRSI;
  emxInit_int32_T(&b_st, &iwork, 1, &nb_emlrtRTEI);
  i1 = iwork->size[0];
  iwork->size[0] = loop_ub;
  emxEnsureCapacity_int32_T(&b_st, iwork, i1, &nb_emlrtRTEI);
  iwork_data = iwork->data;
  for (k = 0; k < loop_ub; k++) {
    iwork_data[k] = 0;
  }
  emxInit_real_T(&b_st, &xwork, 1, &pb_emlrtRTEI);
  i1 = xwork->size[0];
  xwork->size[0] = loop_ub;
  emxEnsureCapacity_real_T(&b_st, xwork, i1, &ob_emlrtRTEI);
